...
export const Galeria = () => {
    ...
    const client = createClient('s80PZl05y2VQ3IiHUeFEZMEOKvYGaHAngMtqVi0XGkl2zxw77tcHlYln');

    useEffect(() => {
        const fetchImages = async () => {
            try {
                if (selectedCountry) {
                    const response = await client.photos.search({ query: selectedCountry.label, per_page: 6 });
